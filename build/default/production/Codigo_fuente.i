...
# 12 "Codigo_fuente.c" 2

# 1 "C:\\Program Files\\Microchip\\xc8\\v2.46\\pic\\include\\c99\\stdbool.h" 1 3
# 13 "Codigo_fuente.c" 2
# 28 "Codigo_fuente.c"
int unid = 0, dece = 0, cent = 0, cont = 0;
...
static const uint8_t display[10] = {0xC0,
                                    0xf9,
                                    0xA4,
                                    0xB0,
                                    0x99,
...
Activar_displays on_display = off;
mensaje mensaje_display = mensaje_espera;
opcion seleccion = ninguno;
    ...
    while(1){
        ...
        if(PORTBbits.RB1 == 0 && state == 0){
            ...
            if(PORTBbits.RB1 == 0 && state == 0){
                ...
                if(seleccion != opcion_50w){
                    seleccion = opcion_35w;
                    mensaje_display = mensaje_35w;
                }

                while(PORTBbits.RB1 == 0 && state == 0){visualizar(0,on_unidad,on_decena,on_centena);}
            }
        }
        ...
        if(PORTBbits.RB2 == 0 && state == 0){
            ...
            if(PORTBbits.RB2 == 0 && state == 0){
                ...
                if(seleccion != opcion_35w){
                    seleccion = opcion_50w;
                    mensaje_display = mensaje_50w;
                }
                while(PORTBbits.RB2 == 0 && state == 0){visualizar(0,on_unidad,on_decena,on_centena);}
            }
        }
        ...
        if(PORTBbits.RB0 == 0){
            ...
            if(PORTBbits.RB0 == 0){
                state = !state;
                while(PORTBbits.RB0 == 0){visualizar(0,on_unidad,on_decena,on_centena);}
            }
        }

        if(state == 1 && mensaje_display != mensaje_espera){
            mensajes();
        }

        if(state == 1 && seleccion == opcion_35w){
            PORTEbits.RE0 = 1;
        }

        if(state == 1 && seleccion == opcion_50w){
            PORTEbits.RE1 = 1;
        }
        ...
        if(state == 0 || seleccion == opcion_35w){
            decremento_time();
        }
        if(state == 0 || seleccion == opcion_50w){
            incremento_time();
        }
        ...
        if(unid == 0 && dece == 0 && cent == 0){
            ...
            visualizar(0,on_unidad,on_decena,on_centena);
        }
    }
}
void config_registros(void){
...
void visualizar(uint8_t VER, uint8_t unidad, uint8_t decena, uint8_t centena){
    ...
    if(on_unidad == unidad){
        ...
        switch(VER){
            case 0:
                ...
                break;
            case 1:
                PORTD = mensaje_on[unid];
                break;
        }
        PORTA = (unsigned char)(~(1<<0));
    ...
    if(on_decena == decena){
        ...
        case 0:
            ...
            break;
        case 1:
            PORTD = mensaje_on[dece];
            break;
        }
        PORTA = (unsigned char)(~(1<<1));
    ...
    if(on_centena == centena){
        ...
        switch(VER){
            case 0:
                ...
                break;
            case 1:
                PORTD = mensaje_on[cent];
                break;
        }
        PORTA = (unsigned char)(~(1<<2));
...
void mensajes(void){
   ...
   switch(mensaje_display){
       case mensaje_espera:
               ...
               dece = 2;
               cent = 0;
               visualizar(1,on_unidad, on_decena, on_centena);

           break;
       case mensaje_35w:
           mens_anterior_35w = milisegundos;
           while(milisegundos - mens_anterior_35w <= 1000){
               unid = 5;
               dece = 3;
               visualizar(0,on_unidad,on_decena,off);
           }
           anterior_35w = milisegundos;
           while(milisegundos - anterior_35w <= 1000){
               unid = 1;
               dece = 0;
               visualizar(1,on_unidad,on_decena,off);
           }
           mensaje_display = mensaje_espera;
           break;
       case mensaje_50w:
           mens_anterior_50w = milisegundos;
           while(milisegundos - mens_anterior_50w <= 1000){
               unid = 0;
               dece = 5;
               visualizar(0, on_unidad, on_decena, off);
           }
           ...
           while(milisegundos - anterior_50w <= 1000){
               unid = 1;
               dece = 0;
               visualizar(1,on_unidad, on_decena, off);
           }
